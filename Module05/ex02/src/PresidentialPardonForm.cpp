...
PresidentialPardonForm::PresidentialPardonForm() : AForm("form", MIN_GRADE_SIGN_PP, MIN_GRADE_EXECUTE_PP), _target("undefinedTarget") {
	std::cout << "PresidentialPardonForm default construcor called." << std::endl;
}

PresidentialPardonForm::PresidentialPardonForm(const std::string & target) : AForm("form", MIN_GRADE_SIGN_PP, MIN_GRADE_EXECUTE_PP), _target(target) {
...
std::ostream &	operator<<(std::ostream & out, const PresidentialPardonForm & SCform){
	out << YELLOW << "[" << SCform.getTarget() << "] Presidential Pardon Form | Grade to sign: " << SCform.getGradeToSign() \
