...
RobotomyRequestForm::RobotomyRequestForm() : AForm("form", MIN_GRADE_SIGN_RR, MIN_GRADE_EXECUTE_RR), _target("undefinedTarget") {
	std::cout << "RobotomyRequestForm default construcor called." << std::endl;
}

RobotomyRequestForm::RobotomyRequestForm(const std::string & target) : AForm("form", MIN_GRADE_SIGN_RR, MIN_GRADE_EXECUTE_RR), _target(target) {
...
std::ostream &	operator<<(std::ostream & out, const RobotomyRequestForm & SCform){
	out << YELLOW << "[" << SCform.getTarget() << "] Robotomy request Form | Grade to sign: " << SCform.getGradeToSign() \
